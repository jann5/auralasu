...
export function ConfigurationPanel() {
    ...
    return (
        <>
        <div className="p-4 md:p-6 lg:p-10 space-y-4 md:space-y-6 pb-48 md:pb-20">
            {/* Header */}
            ...
            <div className={currentStep === 1 ? 'block' : 'hidden'}>
                <h3 className="text-lg md:text-xl font-medium mb-4 md:mb-5">Wybierz Model</h3>
                <div className="space-y-4 md:space-y-5">
                    {MODELS.map((model) => (
                        <div
                            key={model.id}
                            onClick={() => handleModelSelect(model.id)}
                            className={`p-4 md:p-5 min-h-[118px] md:min-h-[138px] border cursor-pointer transition-all duration-200 ${selectedModel === model.id
                                ? 'border-primary-black bg-gray-light'
                                : 'border-gray-medium hover:border-gray-dark'
                                }`}
                        >
                            <div className="flex justify-between items-start gap-2">
                                <span className="font-medium text-lg md:text-xl">{model.name}</span>
                                <span className={`text-sm md:text-base whitespace-nowrap ${selectedModel === model.id ? 'font-medium' : 'text-gray-dark'}`}>
                                    {getPriceDiff(model.basePrice)}
                                </span>
                            </div>
                            <p className="text-sm md:text-base text-gray-dark mt-3 leading-relaxed">{model.description}</p>
                        </div>
                    ))}
                </div>
            </div>
            ...
            <div className={currentStep === 2 ? 'block' : 'hidden'}>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-2 md:gap-4">
                    {WOODS.map((wood) => (
                        ...
                        >
                            <div
                                className="w-full h-16 md:h-24 mb-2 md:mb-3 bg-cover bg-center rounded-sm border border-white/10"
                                style={{
                                    backgroundImage: `linear-gradient(to top right, rgba(0,0,0,0.05), rgba(255,255,255,0.05)), url(${wood.textureUrl})`,
            ...
            <div className="h-64 md:h-16" aria-hidden="true" />
        </div>
